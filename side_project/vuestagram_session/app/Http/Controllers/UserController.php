...
class UserController extends Controller
{
    public function login(Request $request) {
        ...
        $validator = Validator::make(
            ...
            [
                'account' => ['required', 'min:2', 'max:20', 'regex:/^[a-zA-Z0-9]+$/']
        ...
        return response()->json($responseDate, 200)->cookie('auth', '1', 120, null, null, false, false);
    }
    ...
    public function registration(Request $request) {
        ...
        $validator = Validator::make(
            $requestData
            ,[
                'account' => ['required', 'unique:users', 'min:2', 'max:20', 'regex:/^[a-zA-Z0-9]+$/']
                ,'password' => ['required', 'min:2', 'max:20', 'regex:/^[a-zA-Z0-9!@#$%^&*]+$/']
                ,'password_chk' => ['same:password'] // 같은걸 확인할시 required 꼭 안 넣어도 된다
        ...
        // 유효성 검사 실패 체크
        if($validator->fails()) {
            Log::debug('회원 가입 유효성 검사 실패', $validator->errors()->toArray());
            throw new MyValidateException('E01');
        }
